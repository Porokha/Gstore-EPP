...
function gstore_epp_enqueue_assets() {
	...
	$plugin_version = '4.2.0'; // Updated version
	...
	// ============================================
	$typography = get_option('gstore_epp_typography', []);
	if (!empty($typography['source'])) {
...
function gstore_epp_generate_typography_css($typography) {
	...
	// Get font source
	$source = $typography['source'] ?? 'google';
	...
	if ($source === 'google') {
		$css .= "@import url('https://fonts.googleapis.com/css2?family=" .
		        str_replace(' ', '+', $heading_font) . ":wght@{$heading_weight}&family=" .
		        str_replace(' ', '+', $body_font) . ":wght@{$body_weight}&display=swap');\n\n";
	}

	// Load custom font URL if provided
	if ($source === 'url' && !empty($typography['custom_url'])) {
		$css .= "@import url('{$typography['custom_url']}');\n\n";
	}

	// Load uploaded fonts if provided
	if ($source === 'upload') {
		if (!empty($typography['uploaded_fonts'])) {
			$fonts = json_decode($typography['uploaded_fonts'], true);
			foreach ($fonts as $font) {
				$css .= "@font-face {\n";
				$css .= "  font-family: '{$font['family']}';\n";
				$css .= "  src: url('{$font['url']}') format('{$font['format']}');\n";
	...
	$css .= ":host {\n";
	$css .= "  --font-heading: '{$heading_font}', sans-serif;\n";
